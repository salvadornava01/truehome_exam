"""
This module contains the business logic for create, update, and delete Activities
"""
...
from ..utils.custom_exception import ActivityConflict, PropertyInactive, ActivityCancelledOrDone
...
def reschedule_activity(
    ...
    if activity_found.status != 'active':
        raise ActivityCancelledOrDone()
...
def cancel_activity(
    ...
    if activity_found.status != 'active':
        raise ActivityCancelledOrDone()
    activity_found.status = 'done'
    activity_found.save(update_fields=['status'])
    return activity_found
